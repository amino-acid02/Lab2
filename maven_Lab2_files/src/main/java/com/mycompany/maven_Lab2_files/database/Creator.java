...
public class Creator extends ConnectionRunners{
    ...
    //метод запуска Creator - выполнения запроса создания таблиц
    @Override
     public void run(){
        try {
    ...
    //метод выполнения запроса удаления таблиц
    public void dropSQL()
    {
        try {
            Connection engine = DriverManager.getConnection(this.url, this.user, this.password);
    ...
    //метод выполнения запросов по обновлению таблицы units и вычислений
    public void calc()
    {
        try {
            Connection engine = DriverManager.getConnection(this.url, this.user, this.password);
    ...
    //метод для трех способов агрегации
